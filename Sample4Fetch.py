...
def sample():
    ...
    db.result()
    if db.execute('create table ' + tablename + '(idx integer, d1 short, d2 integer, d3 long, f1 float, f2 double, name varchar(20), text text, bin binary, v4 ipv4, v6 ipv6, dt datetime)') is 0:
        return db.result()
    db.result()
...
# append start
    ...
    sStart = time.time()
    if db2.open('127.0.0.1','SYS','MANAGER',port) is 0 :
        return db2.result()
